...
func _ready():
	...
	# HINT: Paksa posisi Gendra pindah ke titik aman SpawnPoint begitu masuk stage 1
	if spawn_point and player:
		...
		print("SISTEM: Gendra berhasil diletakkan di titik spawn aman Stage 1: ", player.global_position)
...
func _on_next_stage_trigger_body_entered(body: Node2D) -> void:
	if body.is_in_group("Player"):
		PlayerState.save(body, "res://scene/stage_002.tscn")
		print("SISTEM: Gendra keluar kamera & menyentuh trigger. Pindah ke Stage 2!")
		
		var next_stage_path = "res://scene/stage_002.tscn"
		
		if ResourceLoader.exists(next_stage_path):
			get_tree().call_deferred("change_scene_to_file", next_stage_path)
		else:
			print("ERROR: File stage_002.tscn tidak ditemukan!")
